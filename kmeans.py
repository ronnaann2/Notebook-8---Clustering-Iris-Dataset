...
class KMeans(object):
    ...
    def get_euclidean_distance(self, point1, point2):
        ...
        if isinstance(point1, pd.Series) and isinstance(point2, pd.Series):
            return np.sqrt(squared_diff.sum().astype(np.float64))
        elif isinstance(point1, pd.DataFrame) and isinstance(point2, pd.Series):
            return np.sqrt(squared_diff.sum(axis=1).astype(np.float64))
        else:
            raise ValueError("Invalid input types. Expected Series and Series, or DataFrame and Series.")
    ...
    def adjust_centroids(self, data, groups):
        ...
        numeric_columns = [col for col in grouped_data.columns if col != 'group' and pd.api.types.is_numeric_dtype(grouped_data[col])]
